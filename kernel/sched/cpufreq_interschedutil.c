...
//Hack it because we dont have CONFIG_SCHED_HISI_TOP_TASK enabled.
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_INTERSCHEDUTIL
static
#endif
struct cpufreq_governor cpufreq_gov_interschedutil;
...
static int sugov_init(struct cpufreq_policy *policy)
{
	...
	ret = kobject_init_and_add(&tunables->attr_set.kobj, &sugov_tunables_ktype,
				   ...
				   cpufreq_gov_interschedutil.name);
	if (ret)
		goto fail;
...
{
	return cpufreq_register_governor(&cpufreq_gov_interschedutil);
}
